...
We also provide a number of [tools](https://github.com/wrightky/MetaViz/blob/main/MetaViz/tools.py) which can help to generally manage collections or their metadata. These are helper functions that could be useful. Example applications include shifting a list of dates by a constant time-delta, stitching together panoramic photos, and geocoding photos based on their location name.
